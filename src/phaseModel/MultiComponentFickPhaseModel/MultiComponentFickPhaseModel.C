    ...
    inertIndex_(-1)
{
    ...
    forAll(Y, i)
    {
        ...
        (
            ...
            (
                ...
                this->mesh(),
                dimensionedScalar("diff", this->mu()->dimensions()/dimDensity, SMALL)
            )
        );
    }

    //- Pointerlist for all diffusivity models
    diffModels_.setSize(1 + this->porosityModels().size());
    ...
    const dictionary& diffDict = dict.subDict(this->name()).subDict("diffusivity");
    ...
    // porous zones
    forAll(this->porosityModels(), iz)
    {
        label znID =
              this->mesh().cellZones().findZoneID(this->porosityModels()[iz].zoneName());

        const labelList& cells = this->mesh().cellZones()[znID];
        const dictionary& porousDiffDict = this->porosityModels()[iz].dict().subDict("diffusivity");
