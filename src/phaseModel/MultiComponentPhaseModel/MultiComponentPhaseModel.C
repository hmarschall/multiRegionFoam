    ...
    inertIndex_(-1)
{
    ...
    diffModels_.setSize(1 + porosityModels().size());
    ...
    // porous zones
    forAll(porosityModels(), iz)
    {
        label znID =
              this->mesh().cellZones().findZoneID(porosityModels()[iz].zoneName());

        const labelList& cells = this->mesh().cellZones()[znID];
        const dictionary& porousDiffDict = porosityModels()[iz].dict().subDict("diffusivity");
