...
namespace Foam
{
    defineTypeNameAndDebug(rotatingGgiFvMesh, 0);
    addToRunTimeSelectionTable(dynamicFvMesh, rotatingGgiFvMesh, IOobject);
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::rotatingGgiFvMesh::addZonesAndModifiers()
{
    // Add zones and modifiers for motion action

    if (cellZones().size() > 0)
    {
        Info<< "void rotatingGgiFvMesh::addZonesAndModifiers() : "
...
void Foam::rotatingGgiFvMesh::calcMovingMasks() const
{
    if (debug)
    {
        Info<< "void rotatingGgiFvMesh::calcMovingMasks() const : "
    ...
    {
        FatalErrorIn("void rotatingGgiFvMesh::calcMovingMasks() const")
...
Foam::rotatingGgiFvMesh::rotatingGgiFvMesh
(
    const IOobject& io,
    ...
    movingPointsMaskPtr_(nullptr)
{
    ...
    if (!cs_.inDegrees())
    {
        WarningIn("rotatingGgiFvMesh::rotatingGgiFvMesh(const IOobject& io)")
...
const Foam::scalarField& Foam::rotatingGgiFvMesh::movingPointsMask() const
{
    if (!movingPointsMaskPtr_)
    {
        calcMovingMasks();
...
bool Foam::rotatingGgiFvMesh::update()
{
