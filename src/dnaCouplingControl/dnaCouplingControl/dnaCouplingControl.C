...
namespace Foam
{
    defineTypeNameAndDebug(dnaCouplingControl, 0);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::dnaCouplingControl::read(const bool absTolOnly)
{
        ...
        if (fieldI == -1)
        {
            ...
            else
            {
                ...
                else
                {
                    FatalErrorIn
                    (
                        "void Foam::dnaCouplingControl::read"
...
Foam::label Foam::dnaCouplingControl::applyToField
(
...
void Foam::dnaCouplingControl::storePrevIterFields() const
{
//    storePrevIter<label>();
...
template<class Type>
void Foam::dnaCouplingControl::maxTypeResidual
(
    const word& fieldName,
    ITstream& data,
    scalar& firstRes,
...
Foam::scalar Foam::dnaCouplingControl::maxResidual
(
    const word& fieldName,
    ITstream& data,
    scalar& lastRes
...
Foam::dnaCouplingControl::dnaCouplingControl(fvMesh& mesh, const word& algorithmName)
:
    IOobject
    (
        "dnaCouplingControl",
...
Foam::dnaCouplingControl::~dnaCouplingControl()
{}
