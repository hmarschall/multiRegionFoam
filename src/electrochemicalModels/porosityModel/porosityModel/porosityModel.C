    ...
    poreDiametre_(100)
{
    ...
    cellZoneIDs_[0] = mesh_.cellZones().findZoneID(zoneName_); //-> this works

    // cellZoneIDs_ = mesh_.cellZones().findIndices(zoneName_); // -> does not work either
    ...
    if
    (
        dict_.readIfPresent("tortuosity", tau_)
     && (tau_ < 1)
    )
    {
        FatalIOErrorIn
    ...
    if
    (
    dict_.readIfPresent("poreDiametre", poreDiametre_)
     && (poreDiametre_ > 1)
    )
    {
        FatalIOErrorIn
        (
            ...
            << "out-of-range poreDiametre_ value " << poreDiametre_
            << exit(FatalIOError);
    }
...
)
{
    ...
    if (!cellZoneIDs_.empty())
    {
        ...
        //.ref() does not exist in foam extend
...
bool Foam::porosityModel::read(const dictionary& dict)
{
    ...
    // cellZoneIDs_ = mesh_.cellZones().findIndices(zoneName_); // Creates a compiler error
