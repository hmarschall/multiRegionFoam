...
:
    ...
    dXYZ_("d", dimless/sqr(dimLength), vector(this->coeffs_.lookup("d"))),
    fXYZ_("f", dimless/dimLength, vector(this->coeffs_.lookup("f"))),
    D_(this->cellZoneIDs_.size()),
    F_(this->cellZoneIDs_.size()),
    rhoName_(this->coeffs_.lookupOrDefault<word>("rho", "thermo:rho")),
    muName_(this->coeffs_.lookupOrDefault<word>("mu", "thermo:mu")),
    nuName_(this->coeffs_.lookupOrDefault<word>("nu", "thermo:nu"))
{
...
void Foam::porosityModels::DarcyForchheimer::calcTransformModelData()
{
    ...
    // {
        forAll(this->cellZoneIDs_, zoneI)
        {
            D_[zoneI].setSize(1);
            F_[zoneI].setSize(1);
    ...
    //     forAll(this->cellZoneIDs_, zoneI)
    //     {
    //         const labelList& cells = mesh_.cellZones()[this->cellZoneIDs_[zoneI]];
    ...
    if (debug && mesh_.time().outputTime())
    {
        ...
        UIndirectList<tensor>(Dout, mesh_.cellZones()[this->cellZoneIDs_[0]]) = D_[0];
        UIndirectList<tensor>(Fout, mesh_.cellZones()[this->cellZoneIDs_[0]]) = F_[0];
