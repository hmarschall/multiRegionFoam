...
Foam::Knudsen<ThermoType>::Knudsen
(
...
:
    ...
    RR_(8314.51),
    PI_(3.14159),    
    dp_("dp", dimensionSet(1,2,-2,-1,0,0,0), readScalar(binaryDiffDict_.lookup("Meanporediametre"))),
    unitsCorr_("units", dimensionSet(-1, 0, 1, 0.5, 0, 0, 0), 1)
{
...
Foam::Knudsen<ThermoType>::D() const
{
   ...
   const fvMesh& mesh = T.mesh();
    ...
    volScalarField& d = tD.ref();
            ...
            d= dp_/3*sqrt((8*RR_*T)/(PI_*W1_))*unitsCorr_;
