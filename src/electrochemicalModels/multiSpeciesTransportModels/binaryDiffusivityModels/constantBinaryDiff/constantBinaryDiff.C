...
Foam::constantBinaryDiff<ThermoType>::constantBinaryDiff
(
...
:
    ...
    Dvalue_("Dvalue", dimensionSet(0, 2, -1, 0, 0,0,0), 0)
{

    word couplename = this->name1_ + word("-") + this->name2_;
    Dvalue_.value() =
        readScalar(binaryDiffDict_.lookup(couplename));
...
Foam::constantBinaryDiff<ThermoType>::D() const
{
