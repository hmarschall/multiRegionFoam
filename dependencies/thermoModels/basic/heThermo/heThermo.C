...
// // This is defined in the base basicThermo
...
// Info << "Print before heCells[celli] " << heCells <<  endl;
    forAll(heCells, celli)
    {
        // rhoCells[celli] =
        //     this->cellMixture(celli).rho(pCells[celli], TCells[celli]);
        // Info << "Print heCells[celli] davor " << heCells[celli] << endl;
        heCells[celli] =
            this->cellMixture(celli).HE(pCells[celli], TCells[celli]);  // Actually HE is called, but due to sensibleEnthalpy missing ...
        // Info << "Print heCells[celli] danach " << heCells[celli] << endl;
    }

    volScalarField::GeometricBoundaryField& heBf = he.boundaryField();
    // Info << "Print init() Ende 1 " << endl;
...
:
    ...
    (
        IOobject
        (
            BasicThermo::phasePropertyName("he", phaseName),
            mesh.time().timeName(),
            mesh,
            IOobject::NO_READ,
            IOobject::NO_WRITE
    ...
    )
{
    ...
    // Info << "Invoke init() in heThermo " << endl;
    // >>>> Problems with init!! -> Gleitkomma Fehler
    init(this->p_, this->T_, he_);

    // Info << "After init() in heThermo " << endl;
    
    // Info << "Print constructor heThermo 1 " << endl;
    // scalarField& hCells = he_.internalField();
    // const scalarField& TCells = this->T_.internalField();

    // Info << "Print constructor heThermo 2 " << endl;
    ...
    // Info << "Print constructor heThermo 3 " << endl;

    // // forAll(h_.boundaryField(), patchi)
    // // {
    // //     h_.boundaryField()[patchi] == h(this->T_.boundaryField()[patchi], patchi);
    // // }
    // Info << "Print constructor heThermo 4 " << endl;

    // // this->hBoundaryCorrection(h_);

    // // calculate();
    // Info << "Print constructor heThermo 5 " << endl;
}
...
{
    tmp<scalarField> the(new scalarField(T.size(), 0));
    // Info << "Print ::he 1 " << endl;
    scalarField& he = the();
    // Info << "Print he davor " << he << endl;
    // Info << "Print ::he 2 " << endl;
...
) const
{
    ...
    forAll(WCells, celli)
    {
        WCells[celli] = this->cellMixture(celli).W();   // In each cell W_mix = \sum_{0}^{i}{Yi*Wi}
    }
...
Foam::heThermo<BasicThermo, MixtureType>::alphahe() const
{
    ...
    // tmp<Foam::volScalarField> alphaEff(this->gamma()*this->alpha_);
    alphaEff().rename("alphahe");
    return alphaEff;
}
...
) const
{
    ...
    // tmp<Foam::volScalarField> alphaEff(this->gamma()*(this->alpha_ + alphat));
