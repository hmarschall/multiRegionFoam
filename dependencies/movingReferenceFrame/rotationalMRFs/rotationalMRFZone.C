...
defineTypeNameAndDebug(Foam::rotationalMRFZone, 0);


const Foam::debug::tolerancesSwitch Foam::rotationalMRFZone::rotAngle_
(
    "rotationalMRFZoneRotAngle",
    10
);
...
void Foam::rotationalMRFZone::setMRFFaces()
{
...
void Foam::rotationalMRFZone::calcMeshVelocity() const
{
    ...
    if (debug)
    {
        InfoIn("void rotationalMRFZone::calcMeshVelocity() const")
    ...
    if (meshVelocityPtr_)
    {
        FatalErrorIn("void rotationalMRFZone::calcMeshVelocity() const")
...
Foam::rotationalMRFZone::rotationalMRFZone(const fvMesh& mesh, Istream& is)
:
    name_(is),
    mesh_(mesh),
    dict_(is),
    ...
    meshVelTime_(-1)
{
    ...
    {
        WarningIn("rotationalMRFZone(const fvMesh&, Istream&)")
    ...
    {
        FatalErrorIn("rotationalMRFZone(const fvMesh&, Istream&)")
    ...
    forAll (excludedPatchNames_, i)
    {
        ...
        if (excludedPatchLabels_[i] == -1)
        {
            FatalErrorIn
            (
                "Foam::rotationalMRFZone::rotationalMRFZone(const fvMesh&, Istream&)"
...
const Foam::surfaceScalarField& Foam::rotationalMRFZone::meshVelocity() const
{
    // Check time
    if
    (
...
Foam::rotationalMRFZone::~rotationalMRFZone()
{
    deleteDemandDrivenData(meshVelocityPtr_);
}
...
Foam::vector Foam::rotationalMRFZone::Omega() const
{
    if (rampTime_ < SMALL)
    {
...
void Foam::rotationalMRFZone::addCoriolis(fvVectorMatrix& UEqn) const
{
    if (cellZoneID_ == -1)
    {
        return;
...
void Foam::rotationalMRFZone::addCoriolis
(
...
void Foam::rotationalMRFZone::addOmega(volVectorField& omg) const
{
    const vector rotVel = Omega();
...
void Foam::rotationalMRFZone::relativeVelocity(volVectorField& U) const
{
...
void Foam::rotationalMRFZone::absoluteVelocity(volVectorField& U) const
{
    const volVectorField& C = mesh_.C();
...
void Foam::rotationalMRFZone::relativeFlux(surfaceScalarField& phi) const
{
    relativeRhoFlux(geometricOneField(), phi);
}


void Foam::rotationalMRFZone::relativeFlux
(
...
void Foam::rotationalMRFZone::absoluteFlux(surfaceScalarField& phi) const
{
    absoluteRhoFlux(geometricOneField(), phi);
}


void Foam::rotationalMRFZone::absoluteFlux
(
    const surfaceScalarField& rho,
    surfaceScalarField& phi
) const
...
void Foam::rotationalMRFZone::meshPhi
(
    surfaceScalarField& phi
) const
{
...
void Foam::rotationalMRFZone::correctBoundaryVelocity(volVectorField& U) const
{
    const vector& origin = origin_.value();
    const vector rotVel = Omega();
...
void Foam::rotationalMRFZone::Su
(
...
#ifdef NoRepository
#   include "rotationalMRFZoneTemplates.C"
#endif
