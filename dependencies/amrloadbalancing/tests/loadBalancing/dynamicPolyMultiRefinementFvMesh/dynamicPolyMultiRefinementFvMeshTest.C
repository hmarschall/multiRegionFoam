...
Description
    Tests for dynamicPolyMultiRefinementFvMesh class
...
namespace multiCriteriaSerial
{
    ...
    //Foam::autoPtr<Foam::Time> timePtr;
...
SCENARIO
(
    "Multi-criteria adaptive poly refinement",
    "[Integration][Serial][!throws]"
)
{
    //// Reset pointer to time
    //resetTimePointer(timePtr, "", "");
    ...
    CAPTURE(maxRefLevel1, maxRefLevel2);
        ...
        )
        {
            ...
            while (runTime.run()) {
                ...
                if (runTime.timeIndex() == 1)
                {
                    ...
                    if(newInterfaceCells.size() <= interfaceCells.size())
