...
const char* const ${typeName}::SHA1sum =
    "${SHA1sum}";
    ...
    )
{
    if (${verbose:-false})
    {
        Info<<"construct ${typeName} sha1: ${SHA1sum} from mesh\n";
    }
}
...
Foam::${typeName}::refinementCellCandidates() const
{
...
Foam::Xfer<Foam::labelList>
Foam::${typeName}::unrefinementPointCandidates() const
{
