...
namespace Foam
{
    defineTypeNameAndDebug(enhancedPrismatic2DRefinement, 0);
...
Foam::label Foam::enhancedPrismatic2DRefinement::getAnchorLevel
(
    const label faceI,
    const label nPoints
) const
{
    ...
    // an error
    // Bugfix: Triangles which have two edges on wege patches have 3 points
    // So, we're allowing this situation
...
void Foam::enhancedPrismatic2DRefinement::appendFaceSplitInfo
(
    const label& faceI,
...
void Foam::enhancedPrismatic2DRefinement::setNewFaceNeighbours
(
    const HashTable
    <
        label,
...
Foam::label Foam::enhancedPrismatic2DRefinement::findMinEdgeConnectedLevel
(
    const label& fpI,
    const label& faceI,
    const face& f,
...
void Foam::enhancedPrismatic2DRefinement::addFaceMids
(
...
void Foam::enhancedPrismatic2DRefinement::checkNewFaceOrientation
(
    polyTopoChange& ref,
    const label& faceI,
    const face& newFace
...
Foam::Pair<Foam::label> Foam::enhancedPrismatic2DRefinement::findSharedWedgePoint
(
    const label& faceI,
    const boolList& edgeOnPatchToCut
) const
{
    const face& f = mesh_.faces()[faceI];
    labelList wedgeEdges;
    const labelList& curEdges = mesh_.faceEdges(faceI);
    forAll (curEdges, i)
    {
        if (edgeOnPatchToCut[curEdges[i]])
        {
            wedgeEdges.append(curEdges[i]);
        }
    }

    // Sanity check
    if (curEdges.size() != 3 || wedgeEdges.size() != 2)
    {
        FatalErrorInFunction
           << "Trying to split a face as it was a triangle between wedge patches."
            << nl
            << "Face: " << faceI << " with number of edges (should be 3): "
            << curEdges.size()
            << " and number of wedge edges (should be 2): "
            << wedgeEdges.size()
            << abort(FatalError);
    }

    // Find point shared by edges on wedge patches
    labelList edgePoints;
    forAll(wedgeEdges, we)
    {
        edgePoints.append(mesh_.edges()[wedgeEdges[we]].start());
        edgePoints.append(mesh_.edges()[wedgeEdges[we]].end());
    }
    sort(edgePoints);
    const auto duplicate = std::adjacent_find(edgePoints.begin(), edgePoints.end());
    const label& pointI = *duplicate;
    label fpI = -1;
    forAll(f, fpi)
    {
        if (f[fpi] == pointI) fpI = fpi;
    }
    return Pair<label>(pointI, fpI);
}

void Foam::enhancedPrismatic2DRefinement::setNewTriangularFaceNeighbours
(
    const HashTable
    <
        label,
...
) const
{
    ...
    {
        FatalErrorInFunction
            << "Trying to compute new own and nei for non-triangular face."
            << nl
            << "Face: " << faceI << " with " << f.size() << " points." 
            << nl
            << abort(FatalError);
    }

    // We need access to two points on the face
    // f[axisPointIndex] is the one on the wedge axis
    // Any other face vertex (pointI here) 
    label pointI = f[0];
    forAll(f, pi)
    {
        if (pi != axisPointIndex)
        {
            pointI = f[pi];
            break;
        }
    ...
    // Get owner cell for this face
    const label ownCellI = mesh_.faceOwner()[faceI];

    // Get cell added cells for owner cell
    const labelList& cAddedOwn = cellAddedCells[ownCellI];

    // Point-owner association for convenience
    const Pair<label> pointOwnerCellPair(pointI, ownCellI);

    // Get new owner for old triangular face using the vertex on the axis
    // If not found on the point-cell map, keep the old owner cell for old face
    if( pointCellToAddedCellMap.found(Pair<label>(f[axisPointIndex], ownCellI)) )
    {
        ownOld = cAddedOwn[pointCellToAddedCellMap[Pair<label>(f[axisPointIndex], ownCellI)]];
    } else {
        ownOld = ownCellI;
    }
    ...
    if (cAddedOwn.empty())
    {
        // old owner cell simply will not be split, keep it as owner for the new face too
        own = ownCellI;
    } else {
        // Old owner cell will be split, fetch the correct cell label using either
        // the vertex on the axis, or the other one
        if( pointCellToAddedCellMap.found(pointOwnerCellPair) )
        {
            own = cAddedOwn[pointCellToAddedCellMap[pointOwnerCellPair]];
        } else if (pointCellToAddedCellMap.found(Pair<label>(f[axisPointIndex], ownCellI))) {
            own = cAddedOwn[pointCellToAddedCellMap[Pair<label>(f[axisPointIndex], ownCellI)]];
        } else {
            FatalErrorInFunction
               << "Expecting to find " << pointOwnerCellPair << " or "
               << Pair<label>(f[axisPointIndex], ownCellI)
               << " in pointCellToAddedCellMap on the owner side." << nl
               << "Face: " << faceI << nl
               << abort(FatalError);
        }
    } 
    ...
    if (mesh_.isInternalFace(faceI))
    {
        // Get neighbour cell for this face
        const label neiCellI = mesh_.faceNeighbour()[faceI];

        // Point-neighbour association for convenience
        const Pair<label> pointNeiCellPair(pointI, neiCellI);

        // Get cell added cells for neighbour cell
        const labelList& cAddedNei = cellAddedCells[neiCellI];

        // Get new neighbour for old triangular face using the vertex on the axis
        // If not found on the point-cell map, keep the old neighbour cell for old face
        if( pointCellToAddedCellMap.found(Pair<label>(f[axisPointIndex], neiCellI)) )
        {
            neiOld = cAddedNei[pointCellToAddedCellMap[Pair<label>(f[axisPointIndex], neiCellI)]];
        } else {
            neiOld = neiCellI;
        }
        ...
        if (cAddedNei.empty())
        {
            // old neighbour cell simply will not be split, keep it as neighbour for the new face too
            nei = neiCellI;
        } else {
            // Old neighbour cell will be split, fetch the correct cell label using either
            // the vertex on the axis, or the other one
            if( pointCellToAddedCellMap.found(pointNeiCellPair) )
            {
                nei = cAddedNei[pointCellToAddedCellMap[pointNeiCellPair]];
            } else if (pointCellToAddedCellMap.found(Pair<label>(f[axisPointIndex], neiCellI))) {
                nei = cAddedNei[pointCellToAddedCellMap[Pair<label>(f[axisPointIndex], neiCellI)]];
            } else {
                FatalErrorInFunction
                   << "Expecting to find " << pointNeiCellPair << " or "
                   << Pair<label>(f[axisPointIndex], neiCellI)
                   << " in pointCellToAddedCellMap on the neighbour side." << nl
                   << "Face: " << faceI << nl
                   << abort(FatalError);
            }
        }
...
void Foam::enhancedPrismatic2DRefinement::splitFaceIntoTwo
(
    const label& faceI,
    const label& anchorLevel,
    const DynamicList<Pair<label>>& splitFacesEmptyEdges,
    const label& splitFacesIntoTwoIndex,
    const boolList& edgeOnPatchToCut,
    HashTable<label,Pair<label>,Hash<FixedList<label,2>>>& pointCellToAddedCellMap,
    labelListList cellAddedCells,
    polyTopoChange& ref,
    labelList& edgeMidPoint,
    boolList& visitedPoint,
    bool modifiedFace
) const
{
    ...
    forAll (f, fpI)
    {
        ...
        )
        {
            ...
            const Pair<label>& edgesOnOppositeSides =
                splitFacesEmptyEdges[splitFacesIntoTwoIndex];
            ...
            const edge& edgeI = mesh_.edges()[edgeIndexI];
            const edge& edgeJ = mesh_.edges()[edgeIndexJ];
            ...
            forAll (pEdges, peI)
            {
                ...
                if (!edgeOnPatchToCut[pointEdgeI])
                {
                    ...
                    const label pointJ =
                        mesh_.edges()[pointEdgeI].otherVertex(pointI);
            ...
            if (edgeMidPoint[mesh_.faceEdges()[faceI][fpI]] == -1)
            {
...
void Foam::enhancedPrismatic2DRefinement::splitWedgeInternalTriangularFace
(
    const label& faceI,
    const label& anchorLevel,
    const DynamicList<Pair<label>>& splitFacesEmptyEdges,
...
) const
{
    ...
    /* wedge -  / \
             | /\  \
              /  \  \ ------- internal/other patch
             /    \  \
             ---------
                 |
                 +---- wedge */

    auto point = findSharedWedgePoint(faceI, edgeOnPatchToCut);
    const label& pointI = point.first();
    const label& fpI = point.second();

    // Convenient interface to otherVertex
    const auto otherEdgePoint = [this](const label& pointI, const label& edgeI)
    {
        return mesh_.edges()[edgeI].otherVertex(pointI);
    };
    ...
    )
    {
        ...
        // New face gets built from the other points and midpoints of wedge patches
        // in this order: (otherOnEdge0 otherOnEdge1 midEdge1 midEdge0)
        // Old face gets modified to represent (sharedVertex midEdge0 midEdge1)
        newFace[0] = otherEdgePoint(pointI, wedgeEdges[0]);
        newFace[1] = otherEdgePoint(pointI, wedgeEdges[1]);
        oldFace[0] = pointI;
        ...
        if ((edgeI.start() == newFace[0]) || (edgeI.end() == newFace[0]))
        {
            oldFace[1] = edgeMidPoint[edgeIndexI];
            oldFace[2] = edgeMidPoint[edgeIndexJ];

        }
        else if
        (
            (edgeJ.start() == newFace[0]) || (edgeJ.end() == newFace[0])
        )
        {
            oldFace[1] = edgeMidPoint[edgeIndexJ];
            oldFace[2] = edgeMidPoint[edgeIndexI];
        }
        else
        {
            // Point not on either of edges, issue an error
            FatalErrorInFunction
               << "Trying to split a face into two, but"
                << " the point: " << newFace[0] << " can't be found"
                << " on either of edges. "
                << nl
                << "Edge: " << edgeIndexI << " with new point: "
                << edgeMidPoint[edgeIndexI]
        ...
        // edgeI is the first edge
        newFace[2] = oldFace[2];
        newFace[3] = oldFace[1];
        
        // The faces are now complete, set new owner/neighbour indices
        // based on split cells for new and old face
        label own, nei, ownOld, neiOld = -1;

        // Set new face owner/neighbour pair
        setNewTriangularFaceNeighbours
        (
            pointCellToAddedCellMap,
            cellAddedCells,
            faceI,
            fpI, // Index of axis point (shared vertex) in the face
        ...
        if (debug)
        {
            Pout.stdStream().clear();
            Pout << "New Face: " << newFace << ", axis point: " << fpI << ", face cell owner: "
                << mesh_.faceOwner()[faceI] << " -> " << cellAddedCells[mesh_.faceOwner()[faceI]]
                << " and cell neighbour: "
                << mesh_.faceNeighbour()[faceI] << " -> " << cellAddedCells[mesh_.faceNeighbour()[faceI]]
                << ", new owner: " << own << " new neighbour: " << nei << endl;
            Pout << "Old Face: " << oldFace  << " " << f << ", new owner: " << ownOld
                << " new neighbour: " << neiOld << endl;

            // Check orientation of the split face for debugging
            checkNewFaceOrientation(ref, faceI, newFace);
        }

        // Modify old face and add the new one
        modifyFace(ref, faceI, oldFace, ownOld, neiOld);
        addFace(ref, faceI, newFace, own, nei);
    }
}

void Foam::enhancedPrismatic2DRefinement::setRefinementInstruction
(
    polyTopoChange& ref
) const
{
    ...
    if (debug)
    {
        ...
        Pout<< "enhancedPrismatic2DRefinement::setRefinementInstruction(...)" << nl
    ...
    // Figure out if a face is triangular and is located between
    // two wedge faces
    auto isTriangleBetweenWedgePatches = [this, &edgeOnPatchToCut](const label& fi) {
        const face& f = mesh_.faces()[fi];
        if (f.size() != 3) return false;
        const labelList& curEdges = mesh_.faceEdges(fi);

        label patchEdgesNbr = 0;
        forAll (curEdges, i)
        {
            if (edgeOnPatchToCut[curEdges[i]]) patchEdgesNbr++;
        }
        if (patchEdgesNbr == 2) return true;
        return false;
    };

    // Figure out if a cell is a wedge cell at axis
    auto isWedgeAxisCell = [this, &isTriangleBetweenWedgePatches](const label& cellI)
    {
        bool isAxisCell = false;
        forAll(mesh_.cells()[cellI], ci)
        {
            if (isTriangleBetweenWedgePatches(mesh_.cells()[cellI][ci]))
            {
                isAxisCell = true;
                break;
            }
    ...
    forAll(refineCellsMask, cellI)
    {
        ...
        if (refineCellsMask[cellI])
        {
            ...
            // Bugfix, OK but the cells which have two faces on wedge patch
            // Abuse this rule
            else if (cellAnchorPointsDynamic[cellI].size() % 2 != 0 && !isWedgeAxisCell(cellI))
            {
    ...
    forAll(faceMidPoint, faceI)
    {
        //if (isTriangleBetweenWedgePatches(faceI) && faceMidPoint[faceI] > -1)
        //{
        //    faceMidPoint[faceI] = -1;
        //    // This is a face that will be split into two
        ...
        //            // Find the point which is shared by wedge edges
        //            const label anchorI = findSharedWedgePoint(faceI, edgeOnPatchToCut).first();
        ...
        //                }
        //                else if (cellCounter < cAdded.size())
        //                {
    ...
    if (debug)
    {
        Pout<< "enhancedPrismatic2DRefinement::setRefinementInstruction(...)" << endl
    ...
    forAll (splitFacesIntoTwo, i)
    {
        ...
        if (facesToSplit[faceI])
        {
            ...
            // Additional sanity check
            // Bugfix: Allow trianglar faces, they can get split into a quad and a triangle
            if (f.size() != 4 && f.size() != 3)
            {
                FatalErrorInFunction
                   << "The original face has: " << f.size() << " points,"
                    << " while it should have 4 or 3 points in order"
                    << " to split it in two."
                    << " faceI: " << faceI
                    << abort(FatalError);
            }
            ...
            if (isTriangleBetweenWedgePatches(faceI)) {
                splitWedgeInternalTriangularFace
                (
                    faceI,
                    anchorLevel,
                    splitFacesEmptyEdges,
    ...
    forAll(faceMidPoint, faceI)
    {
        ...
        if (faceMidPoint[faceI] > -1 && cellsToSplit[cellI])
        {
            ...
            forAll (f, fpI)
            {
                ...
                // Bugfix, some internal faces might be invalid cuz of the wedging thing
                label invalidIndex = -1;
                forAll(newFace, fpI)
                {
                    if (newFace[fpI] == -1) invalidIndex = fpI;
                }
...
void Foam::enhancedPrismatic2DRefinement::setUnrefinementInstruction
(
    polyTopoChange& ref
) const
{
    ...
    if (debug)
    {
        Pout<< "enhancedPrismatic2DRefinement::setUnrefinementInstruction"
            << "(polyTopoChange& ref)"
            << nl
    ...
    // Memory management
    {
        ...
        if (facesToRemove.size() != splitFaces.size())
        {
            FatalErrorIn
            (
                "enhancedPrismatic2DRefinement::setUnrefinementInstruction"
                "(polyTopoChange& ref)"
...
Foam::enhancedPrismatic2DRefinement::enhancedPrismatic2DRefinement
(
    const word& name,
...
:
    enhancedRefinement(name, dict, index, mme)
{}
...
void Foam::enhancedPrismatic2DRefinement::setCellsToRefine
(
...
)
{
    if (debug)
    {
        Info<< "enhancedPrismatic2DRefinement::setCellsToRefine"
...
void Foam::enhancedPrismatic2DRefinement::setSplitPointsToUnrefine
(
...
)
{
    if (debug)
    {
        Info<< "enhancedPrismatic2DRefinement::setSplitPointsToUnrefine"
    ...
    for (label i = 0; i < nUnrefinementBufferLayers_; ++i)
    {
        ...
        // bugFix: 
        // extend marked cells without getting unnecessary proc-proc boundaries
        // involved, MEF, 2021
        //procReadyExtendMarkedCellsAcrossPoints(mesh_, protectedCell);
    }
